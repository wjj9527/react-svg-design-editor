...
const actions: ActionsType = {
  ...
  [TYPES.SET_NODE_RESIZE_MOVE_ATTRIBUTE]: (state, action) => {
    let { currentAction, schema, svgOffset, canvasScale } = state;
    ...
    if (id) {
      ...
      const setValue = (value: number) => {
        ...
        return settingValue / canvasScale;
      };
      if (type === 'RESIZE') {
        switch (target) {
          case 'RIGHT_TOP':
            width = setValue(pageX - targetNode.x * canvasScale - svgOffset.x);
            height = setValue(
              targetNode.height * canvasScale +
                targetNode.y * canvasScale -
                pageY +
                svgOffset.y,
            );
          ...
          case 'RIGHT_BOTTOM':
            width = setValue(pageX - targetNode.x * canvasScale - svgOffset.x);
            height = setValue(pageY - targetNode.y * canvasScale - svgOffset.y);
            targetNode.width = width;
            targetNode.height = height;
            break;
          case 'CENTER_TOP':
          ...
          case 'LEFT_TOP':
            ...
            width = setValue(
              targetNode.width * canvasScale +
                targetNode.x * canvasScale -
                pageX +
                svgOffset.x,
            );
